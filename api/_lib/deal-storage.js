import { kvGet, kvSet, kvDel, kvKeys } from './storage.js';
...
export async function listDeals(filter = {}) {
  const keys = await kvKeys('deal:DL-*');
  const deals = (await Promise.all(keys.map(k => kvGet(k)))).filter(Boolean);
...
export async function seedDeals() {
  ...
  for (const d of DEALS) {
    const exists = await kvGet(`deal:DL-SEED-${d.name.slice(0,3).toUpperCase()}`);
    if (!exists) {
      const id = 'DL-' + d.name.replace(/[^A-Z]/gi,'').slice(0,4).toUpperCase() + '1';
      const deal = { id, ...d, advisor_id:'adv-sg1', advisor_admin_mode:false, qa:[], audit_log:[], created_at:new Date().toISOString(), updated_at:new Date().toISOString() };
      await kvSet(`deal:${id}`, deal);
      results.push(id);
    }
  }
  return results;
