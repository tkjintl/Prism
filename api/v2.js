...
Return ONLY valid JSON in this exact structure:
{
  ...
  "geography": "e.g. Asia-Pacific"
    ...
    // ── admin-create-from-docs: upload docs → AI extracts + generates → creates deal ──
    if (op === 'admin-create-from-docs') {
      const adminPayload = await getAdmin();
      if (!adminPayload) return unauth(res);
      const { tempId } = req.body || {};
      if (!tempId) return bad(res, 'tempId required');

      const slots = ['nda', 'mgmt', 'fin', 'term'];
      const docContent = [];
      for (const slot of slots) {
        const doc = await kvGet(`pdoc_admin:${tempId}:${slot}`);
        if (doc?.data) docContent.push({ slot, name: doc.name, type: doc.type || 'application/pdf', data: doc.data });
      }
      if (docContent.length === 0) return bad(res, 'No documents found — upload at least one document');

      // BOT_MODE: return mock so bot tests exercise the path without burning tokens
      const now = new Date().toISOString();
      if (process.env.BOT_MODE === '1') {
        const mockId = 'DL-ADM-' + Date.now().toString(36).toUpperCase();
        const mockDeal = { id: mockId, name: '[BOT] Mock Admin Deal', asset_class: 'credit', deal_structure: 'Senior Secured', geography: 'Asia-Pacific', originator: 'TACC Singapore', tagline: '[BOT-MODE mock] Admin-created deal', company_overview: 'Bot-mode placeholder. Not real. AI was not called.', thesis: 'Bot-mode placeholder thesis for admin-created deal. Not generated by Anthropic — BOT_MODE is on.', highlights: [{ icon: '◆', s: 'Mock highlight', b: 'Synthetic content for bot-test mode.' }, { icon: '◆', s: 'Mock highlight 2', b: 'Bot-mode only.' }], target_irr: 12, hurdle_rate: 8, term_months: 24, target_alloc_usd: 10000000, min_ticket_usd: 250000, closing_date: new Date(Date.now() + 180 * 86400000).toISOString(), stage: 'review', member_visible: false, ioi_count: 0, ioi_agg_usd: 0, created_at: now, updated_at: now };
        return ok(res, { ok: true, extracted: mockDeal, mocked: true });
      }

      if (!process.env.ANTHROPIC_API_KEY) {
        const mockExtracted = { name: 'Admin Deal (Dev Mock)', asset_class: 'credit', deal_structure: 'Senior Secured Bridge', geography: 'Southeast Asia', originator: 'TACC Singapore', tagline: 'Senior secured bridge financing for institutional investors in Southeast Asia', company_overview: 'A senior secured bridge financing opportunity originated by TACC Singapore targeting institutional and family office investors in the Asia-Pacific region. The transaction provides first-lien security over the underlying assets.', thesis: 'This transaction presents a compelling risk-adjusted return profile for sophisticated investors seeking yield in the Asian credit markets. The senior secured structure provides downside protection while delivering attractive carry relative to comparable instruments in the region.', highlights: [{ icon: '◆', s: 'Senior secured structure', b: 'First-lien security over underlying assets provides principal protection in a downside scenario.' }, { icon: '◆', s: 'Attractive yield', b: 'Floating-rate instrument with a spread premium over regional benchmarks.' }], target_irr: 13, hurdle_rate: 8, term_months: 24, target_alloc_usd: 20000000, min_ticket_usd: 500000, closing_date: new Date(Date.now() + 120 * 86400000).toISOString().slice(0, 10) };
...
{
  "name": "Full deal/fund name as it appears in documents",
  "asset_class": "credit|pe|re|infra",
  "deal_structure": "e.g. Senior Secured Bridge Loan",
  "geography": "e.g. Southeast Asia",
  "originator": "Sponsor or originator company name",
  "target_irr": 14.5,
  "hurdle_rate": 8.0,
  "term_months": 36,
  "target_alloc_usd": 30000000,
  "min_ticket_usd": 500000,
  "closing_date": "YYYY-MM-DD",
  "tagline": "One compelling sentence — deal name, geography, key differentiator",
  "company_overview": "2–3 sentences describing the company/sponsor and transaction. Minimum 80 characters.",
  "thesis": "3–4 paragraphs. Investment rationale, market opportunity, risk/return profile. Institutional tone. Minimum 200 characters.",
  "highlights": [
    { "icon": "◆", "s": "Short bold title (4-6 words)", "b": "One sentence expanding on the point." },
    { "icon": "◆", "s": "Short bold title", "b": "One sentence." },
    { "icon": "◆", "s": "Short bold title", "b": "One sentence." }
  ]
}
...
- target_alloc_usd and min_ticket_usd in full dollars (e.g. 30000000 not 30M)
- closing_date as YYYY-MM-DD; if unclear, use a date 6 months from today
- asset_class must be exactly one of: credit, pe, re, infra
- highlights: 3–5 items minimum
- If a field cannot be found in the documents, use a reasonable placeholder but flag it`;

      try {
        const result = await callAI(
          [{ role: 'user', content: [...docBlocks, { type: 'text', text: userPrompt }] }],
          { model: 'claude-sonnet-4-6', maxTokens: 2000, system: systemPrompt, extraHeaders: { 'anthropic-beta': 'pdfs-2024-09-25' } },
        );
        const rawText = result.content?.[0]?.text || '';
        let extracted;
        try {
          ...
          return bad(res, 'Failed to parse AI response — check document quality', 500);
        }
        return ok(res, { ok: true, extracted, tempId });
      } catch (err) {
        console.error('[admin-create-from-docs] AI error:', err);
        return bad(res, 'AI extraction failed: ' + (err?.message || 'unknown'), 500);
      }
    }

    // ── admin-confirm-new-deal: save the confirmed AI-extracted deal ──
    if (op === 'admin-confirm-new-deal') {
      const adminPayload = await getAdmin();
      if (!adminPayload) return unauth(res);
      const { tempId, dealData } = req.body || {};
      if (!dealData) return bad(res, 'dealData required');

      let deal;
      try {
        deal = await createDeal(dealData, null, adminPayload.email);
      } catch (e) {
        if (e?.code === 'DEAL_VALIDATION') return res.status(400).json({ ok: false, error: e.message, missing: e.missing });
        throw e;
      }

      // Move temp docs to permanent deal_doc keys
      if (tempId) {
